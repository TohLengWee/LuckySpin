function CWheel (iX, iY){
    ...
    this.setText = function(iMultiply){
        ...
        for(var i=0; i<PRIZE.length; i++ ){ 
            // var nomor = (PRIZE[i] + "").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
            if (i%5){
            _aText[i] = new CFormatText(vVect.getX(), vVect.getY(), TEXT_CURRENCY + PRIZE_NAME[i], _oWheelContainer,"#fff");
